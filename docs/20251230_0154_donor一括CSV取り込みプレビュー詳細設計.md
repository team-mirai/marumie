...
export class DonorCsvValidator {
  validate(
    ...
    return rows.map((row) => this.validateRow(row, transactionMap));
  }
...
export class PreviewDonorCsvUsecase {
  ...
  async execute(input: PreviewDonorCsvInput): Promise<PreviewDonorCsvResult> {
    ...
    // 5. 既存 Donor 検索（name + address + donorType で一致するものを探す）
  ...
  private async enrichWithMatchingDonors(
    ...
    // 有効な donorType を持つ行のみ検索
    const searchTargets = rows.filter((row) => row.donorType !== null);
    ...
    const enrichedRows = await Promise.all(
      rows.map(async (row) => {
        ...
        const matchingDonor = await this.donorRepository.findByNameAddressAndType(
          row.name,
          row.address,
          row.donorType,
        );
...
- [x] actionsで適切に`revalidatePath`を呼び出している（プレビューは読み取り専用のため不要）
