...
**メンテナンス画面の内容:**
- シンプルなHTMLで「メンテナンス中」を表示
- サービス名とメンテナンス中である旨のメッセージ
- 外部リソース（CSS/JS/画像）への依存なし
...
**注意**: 環境変数変更後、再デプロイは不要（Edge Runtimeは環境変数を動的に読み取る）
...
1. Vercel Dashboardで `MAINTENANCE_MODE="true"` を設定（webapp/admin両方）
2. 数秒後にメンテナンスモードが有効化される
3. DBマイグレーション等の作業を実施
