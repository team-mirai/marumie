...
#### 寄附関連 (SYUUSHI07_07に対応)
- `income_donation_individual` (個人からの寄附)
- `income_donation_corporation` (法人その他の団体からの寄附)
- `income_donation_political_org` (政治団体からの寄附)
- `income_donation_intermediary_individual` (あっせんによる寄附 - 個人)
- `income_donation_intermediary_corporation` (あっせんによる寄附 - 法人)
- `income_donation_intermediary_political_org` (あっせんによる寄附 - 政治団体)

#### 政治資金パーティー関連 (SYUUSHI07_11に対応)
- `income_party_individual` (個人からの対価支払)
- `income_party_corporation` (法人その他の団体からの対価支払)
- `income_party_political_org` (政治団体からの対価支払)
- `income_party_intermediary_individual` (あっせんによる対価支払 - 個人)
- `income_party_intermediary_corporation` (あっせんによる対価支払 - 法人)
- `income_party_intermediary_political_org` (あっせんによる対価支払 - 政治団体)
...
1. 匿名寄附
   - `income_anonymous_donation` (政党匿名寄附 - SYUUSHI07_09) ※寄附者が特定できないためdonorを紐付けられない
2. 非寄附関連の収入
   - `income_business`
   - `income_loan`
...
## donor_type と category_key の整合性チェック

donorの種別と取引のcategory_keyには整合性が必要となる。以下のマッピングを強制する:

| donor_type | 許可されるcategory_key（正規表現） |
|-----------|--------------------------------|
| individual | `income_(donation\|party)(_intermediary)?_individual` |
| corporation | `income_(donation\|party)(_intermediary)?_corporation` |
| political_organization | `income_(donation\|party)(_intermediary)?_political_org` |

**例:**
- OK: `donor_type='individual'` かつ `category_key='income_donation_individual'`
- NG: `donor_type='individual'` かつ `category_key='income_donation_corporation'`
...
3. donor_typeとcategory_keyの整合性が取れていること
...
DECLARE
  ...
  v_allowed_donation_categories TEXT[] := ARRAY[
    ...
    'income_donation_intermediary_individual',
    'income_donation_intermediary_corporation',
    'income_donation_intermediary_political_org'
  ];
  v_allowed_party_categories TEXT[] := ARRAY[
    'income_party_individual',
    'income_party_corporation',
    'income_party_political_org',
    'income_party_intermediary_individual',
    'income_party_intermediary_corporation',
    'income_party_intermediary_political_org'
  ];
  v_all_allowed_categories TEXT[];
BEGIN
  -- 全許可カテゴリを結合
  v_all_allowed_categories := v_allowed_donation_categories || v_allowed_party_categories;
...
| 職業情報 | 個人の場合に保持 | なし |
| 対象取引 | income_donation_*, income_party_* | expense_*, income_business, income_loan等 |
...
### 5. donor_typeとcategory_keyの整合性
- triggerにより、individual donorがcorporation向けcategoryに紐付くことを禁止
- 例: donor_type='individual'のdonorは、category_key='income_donation_corporation'に紐付け不可
...
- 政治資金収支報告書 XMLデータフォーマット仕様書: [docs/report-format.md](report-format.md)
