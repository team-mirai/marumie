...
export interface AssignDonorResult {
  success: boolean;
  errors?: string[];
}

export class AssignDonorUsecase {
  constructor(
  ...
  async execute(input: AssignDonorInput): Promise<AssignDonorResult> {
    ...
    const transaction = await this.transactionRepository.findByIdWithDonor(transactionBigIntId);
    if (!transaction) {
      return { success: false, errors: ["トランザクションが見つかりません"] };
    }

    if (!isDonorTypeAllowedForCategory(transaction.categoryKey, donor.donorType)) {
      return {
        success: false,
        errors: [
          `このカテゴリ（${transaction.categoryKey}）には${donor.donorType}タイプの寄付者を紐付けできません`,
        ],
      };
    }

    await this.transactionDonorRepository.upsert(transactionBigIntId, donorBigIntId);
